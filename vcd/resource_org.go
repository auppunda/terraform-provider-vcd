...
func resourceOrgCreate(d *schema.ResourceData, m interface{}) error {
	...
	//log.Printf("Finding vdc network: %s", d.Get("network_name"))
	//net, err := vcdClient.OrgVdc.FindVDCNetwork(d.Get("network_name").(string))

	// if err != nil {
	// 	log.Printf("Error finding VCD network: %#v", err)
	// 	return fmt.Errorf("Error finding VCD network: %#v", err)
	// }
...
func resourceOrgDelete(d *schema.ResourceData, m interface{}) error {
	...
	log.Printf("Disabling Org with id %s", d.State().ID)
	_, err := vcdClient.DisableOrg(d.State().ID)
	if err != nil {
		return err
	}
