...
func resourceOrgCreate(d *schema.ResourceData, m interface{}) error {
	...
	task, err := govcloudair.CreateOrg(vcdClient.VCDClient, orgName, fullName, isEnabled, canPublishCatalogs, vmQuota)
...
func resourceOrgDelete(d *schema.ResourceData, m interface{}) error {
	...
	org, err := govcloudair.GetAdminOrgById(vcdClient.VCDClient, d.State().ID)
	if err != nil {
...
func resourceOrgRead(d *schema.ResourceData, m interface{}) error {
	...
	log.Printf("Reading org with id %s", d.State().ID)
	_, err := govcloudair.GetAdminOrgById(vcdClient.VCDClient, d.State().ID)
